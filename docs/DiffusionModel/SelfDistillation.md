...
> [!note]
>
> 首先注意，这个 x 指的都是 $x_s$ ，即 s 时刻的起始隐状态，那么左边指的是，这个点映射 到 t 时刻后，对于 t 求偏导，那么很好理解，就是 $\dot x_t$ ，t 时刻的瞬时速度。而右边 这个 $b_t$ 的 t 都不算个参数，实际求的是 $b_t(x_s) = \dot x_s$ 也就是 s 时刻的瞬时速度。这个符号混用给我二心坏了
