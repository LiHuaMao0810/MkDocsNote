...
分析标准 Attention 的性能问题，必须从 **GPU 的硬件架构**出发，而不是单看数学计算量。

GPU 内部有两种存储层级：

| 存储类型             | 容量                          | 速度                               |
| -------------------- | ----------------------------- | ---------------------------------- |
| **HBM（显存）**      | 约 40–80 GB                   | 慢（带宽约 2 TB/s）                |
| **SRAM（片上缓存）** | 每个 SM 几百 KB，总计约 20 MB | 极快（带宽约 20 TB/s，快约 10 倍） |

CUDA Core 的计算速度远超 HBM 的传输速度。因此，标准 Attention 的瓶颈不是"算得慢"，而是"等数据"——GPU 大部分时间处于 **Memory-bound（内存带宽受限）** 状态。

**标准 Attention 的 HBM 读写流程：**

1. 从 HBM 读取 $Q, K $ → 计算注意力分数矩阵 $S = QK^T $（形状为 $L \times L $）
2. 将 $S $ **写回 HBM**（SRAM 容量不足，无法保存整个矩阵）
3. 从 HBM 重新读取 $S $ → 执行 Softmax → 结果写回 HBM
4. 从 HBM 读取 Softmax 结果和 $V $ → 计算 $\text{Attention}(Q,K,V) $ → 写回 HBM

整个过程中，$S $ 矩阵被反复在 HBM 和 SRAM 之间搬来搬去，产生了大量低效的 IO。 **HBM 访问量随序列长度呈 $O(L^2) $ 增长。**

------

### 技术一：分块计算（Tiling）

Flash Attention 的核心思路：**既然 HBM 搬运慢，就让计算尽量留在 SRAM 内完成。**

将 $Q, K, V $ 矩阵切分成若干小块（Tiles），每次只从 HBM 读入一小块，确保其大小可以放入 SRAM。在 SRAM 内完成该块的全部计算后，只将最终输出结果写回 HBM， **中间的 $S $ 矩阵从不写回显存** 。

这样一来：

- 每个分块只产生一次 HBM 读入 + 一次 HBM 写出
- **HBM 访问量从 $O(L^2) $ 降至 $O(L) $**，计算量本身（$O(L^2) $）不变

> **注意：** Flash Attention 优化的是 **HBM IO 次数**，而非数学计算复杂度。计算量仍为 $O(L^2) $，但瓶颈已从"搬运"转移到"计算"，GPU 利用率因此大幅提升。

**分块带来的新问题：Online Softmax**

Softmax 需要全局的最大值和求和项才能归一化，但分块后每次只能看到局部数据。Flash Attention 通过 **Online Softmax** 解决这一问题：在逐块扫描时，动态维护并更新全局最大值 $m$ 和归一化因子 $\ell $，每处理完一个新块就对之前的累积结果进行修正。数学上可以证明，这种迭代修正收敛到与全局 Softmax 完全等价的结果。

![image-20260327204104174](./assets/image-20260327204104174.png)

------

### 技术二：反向传播重算（Recomputation）

**为什么 $S $ 矩阵会成为显存杀手？**

以 Llama-3-8B 处理 32k 上下文为例（$L = 32768, D = 4096 $）：

| 矩阵               | 形状      | 元素数量           | FP16 显存占用     |
| ------------------ | --------- | ------------------ | ----------------- |
| $Q $（或 $K, V $） | $(L, D) $ | $\approx 1.34 $ 亿 | $\approx 256 $ MB |
| $S $（注意力分数） | $(L, L) $ | $\approx 10.7 $ 亿 | $\approx 2 $ GB   |

若上下文扩展至 128k，$Q/K/V $ 增长 4 倍，而 $S $ 增长 **16 倍**。128k 下单个 $S $ 矩阵用 FP16 存储需约 **32 GB 显存**，超出单卡容量上限。

**标准反向传播的做法**是在前向传播时将 $S $ 保存在显存中，以便求导时直接使用。Flash Attention 选择完全抛弃这一做法：

- **前向传播：** 仅保存 $(L, D) $ 大小的 $Q, K, V $ 及 Softmax 归一化因子（一个 $O(L) $ 的向量），$S $ 矩阵计算后立即丢弃。
- **反向传播：** 重新读取 $Q, K, V $，按分块方式在 SRAM 内现场重算出局部 $S $，完成梯度计算后立即销毁，逐块处理直至完成。

**反直觉的结论：** 现场重算的额外计算开销，比将 $S$ 持续存写 HBM 所产生的 IO 开销更小。这再次印证了现代 GPU 是计算快、IO 慢的架构特性。
...
在解释 GQA 和 MQA 之前，先回顾标准的 **Multi-Head Attention (MHA)**：

![image-20260327202120329](./assets/image-20260327202120329.png)

**MHA（Multi-Head Attention）** 是标准 Transformer 的注意力机制。每个注意力头都有自己独立的 Q、K、V 投影矩阵，4 个头就有 4 套 K/V。这在推理时带来一个大问题：为了避免重复计算，需要把所有层的 K/V 缓存到显存（即 **KV Cache**），头数越多、序列越长，显存占用就越爆炸。

------

**MQA：Multi-Query Attention**

由 Noam Shazeer 在 2019 年提出，思路极其激进：**所有 Q 头共享同一组 K 和 V**。

- KV Cache 从 `N×(K+V)` 直接降到 `1×(K+V)`，内存节省接近 N 倍
- 推理速度显著提升，因为显存带宽压力大幅降低
- 代价是模型质量有所下降，尤其在需要多样化注意力模式的任务上表现变差
- PaLM、Falcon 等模型使用了 MQA

------

**GQA：Grouped Query Attention**

由 Google 在 2023 年提出，是 MHA 和 MQA 的折中方案：**将 Q 头分成 G 个组，每组共享一套 K/V**。

- 当 G = 1（1 个组），退化为 MQA
- 当 G = N（组数等于头数），退化为 MHA
- 实践中通常取 G = N/4 或 N/8，在质量和效率之间取得很好的平衡

GQA 现在已成为主流大模型的标配，**LLaMA 2/3、Mistral、Gemma、Qwen** 等都在使用。
